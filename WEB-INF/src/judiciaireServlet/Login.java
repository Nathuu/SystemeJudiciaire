...
/**
 * Classe pour login système de gestion judiciare
 * <P>
...
public class Login extends HttpServlet
{
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        try
        {
            ...
            // fermer la session si elle a déjà été ouverte lors d'un appel précédent
            ...
            if (session.getAttribute("etat") != null)
            {
                ...
                System.out.println("GestionJudiciaire: session déja crée; id=" + session.getId());
                // la méthode invalidate appelle le listener BiblioSessionListener; cette classe est chargée lors du
                // démarrage de
                // l'application par le serveur (voir le fichier web.xml)
                session.invalidate();
                session = request.getSession();
                System.out.println("GestionJudiciaire: session invalidée");
            }
